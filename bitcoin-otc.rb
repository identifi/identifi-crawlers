...
require 'net/http'
require 'json'
require './identifi-rpc.rb'

module BitcoinOTCCrawler
  ...
  def downloadRatingDetails(http)
    request = Net::HTTP::Get.new ratingdetailurl(username)
  ...
  def downloadViewGPG(http)
    request = Net::HTTP::Get.new viewgpgurl(username)
    response = http.request request # Net::HTTPResponse object
  ...
  def addToIdentifi
    ...
    Dir.foreach(VIEWGPG_DIR) do |item|
      next if item == '.' or item == '..'
      File.open( item, "r" ) do |f|
        json = JSON.load( f )
      end
